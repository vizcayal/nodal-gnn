...
class GATEdgePredictor(torch.nn.Module):
    ...
    Architecture:
        ...
        2. N x (GATConv -> BatchNorm1d -> ReLU -> Dropout) + skip connection
        3. Edge MLP: concat(h_u, h_v) -> hidden -> 1
    """

    def __init__(self, in_channels, hidden_channels, branch_u, branch_v,
                 dropout_rate=0.2, num_layers=5, heads=4, num_edge_feats=0):
        super().__init__()
        ...
        # Deep GAT blocks
        # Setting concat=False ensures output dim = hidden_channels, allowing residual connection sum
        self.convs = nn.ModuleList(
            [GATConv(hidden_channels, hidden_channels, heads=heads, concat=False, dropout=dropout_rate) 
